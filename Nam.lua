...
-- ========= AUTO DEFENSE =========
-- 1) RemoteEvent listener
if Remote then
    Remote.OnClientEvent:Connect(function(thief)
...
-- 2) ProximityPrompt listener for brainrots
local function hookBrainrot(brainrot)
    ...
    prompt.Triggered:Connect(function(thief)
        ...
        if cooldown[thief] and os.clock() - cooldown[thief] < COOLDOWN_TIME then return end
        cooldown[thief] = os.clock()
        print("[AutoDefense] Spamming all commands on thief:", thief.Name)
        runSpam(thief)
    end)
end

-- Hook existing brainrots
