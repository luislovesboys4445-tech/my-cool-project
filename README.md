...
-- Commands (added ragdoll & nightvision)
local Commands = {
    ...
    ragdoll = function(target) Remote:FireServer(target,"ragdoll") end,
    nightvision = function(target) Remote:FireServer(target,"nightvision") end,
}

-- Command order (so ragdoll & nightvision spam with others)
local CommandOrder = {"rocket","balloon","inverse","tiny","morph","jumpscare","ragdoll","nightvision"}
...
frame.Name = "Root"
frame.Size = UDim2.fromOffset(200, 264)
...
holder.Position = UDim2.fromOffset(0,30)
holder.Parent = frame
