...
-- ================= COMMANDS =================
local Commands = {
	balloon = function(t) Remote:FireServer(t, "balloon") end,
	rocket = function(t) Remote:FireServer(t, "rocket") end,
	inverse = function(t) Remote:FireServer(t, "inverse") end,
	tiny = function(t) Remote:FireServer(t, "tiny") end,
	morph = function(t) Remote:FireServer(t, "morph") end,
	jumpscare = function(t) Remote:FireServer(t, "jumpscare") end,
	nightvision = function(t) Remote:FireServer(t, "nightvision") end,
}
local CommandOrder = {"rocket","balloon","inverse","tiny","morph","jumpscare","nightvision"}
...
-- Spam function (0 delay for all except rocket = 0.15)
local function runSpam(target)
	...
	for _, cmd in ipairs(CommandOrder) do
		...
		if cmd == "rocket" then task.wait(0.15) end -- rocket delay
	end
end

-- ========= PLAYER BUTTON MANAGEMENT =========
...
local function removePlayerButton(p)
	...
	cooldown[p] = nil -- remove from cooldown
end
...
-- ========= AUTO DEFENSE =========
local BrainrotStealRemote = ReplicatedStorage:WaitForChild("StealBrainrot")
...
BrainrotStealRemote.OnClientEvent:Connect(function(thief)
	...
	-- Convert thief to Player object if needed
	...
	-- Debug print
	print("[AutoDefense] Triggered on:", thief.Name)
